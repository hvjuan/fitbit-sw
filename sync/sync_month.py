"""Sync a full month of sleep data from Fitbit API to MariaDB.

This script syncs sleep data for all days in a specified month.
...
import lib.auth
import lib.client
from sync_sleep import SleepSync
...
def main():
    ...
    if len(sys.argv) < 3:
        print('Usage: python sync_month.py <year> <month>')
        print('Example: python sync_month.py 2025 10')
        sys.exit(1)
    ...
    # Sync sleep data for the entire month
    ...
    try:
        ...
        print(f'\n📅 Syncing sleep data for {year}-{month:02d} ({len(dates)} days)\n')
        ...
        print(f'\n✓ Month sync complete!')
        print(f'  Total sessions synced: {total_synced}')
